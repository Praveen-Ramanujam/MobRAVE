...
public:
      ...
      class PyTouchSensorData : public PySensorData
    {
public:
        PyTouchSensorData(boost::shared_ptr<SensorBase::TouchGeomData> pgeom, boost::shared_ptr<SensorBase::TouchSensorData> pdata) : PySensorData(pdata)
        {
            positions = toPyArray3(pdata->positions);
            ranges = toPyArray3(pdata->ranges);
            intensity = toPyArrayN(pdata->intensity.size()>0 ? &pdata->intensity[0] : NULL,pdata->intensity.size());
        }
        PyTouchSensorData(boost::shared_ptr<SensorBase::TouchGeomData> pgeom) : PySensorData(SensorBase::ST_Touch) {
        }
        virtual ~PyTouchSensorData() {
        }
...
public:
    ...
    boost::shared_ptr<PySensorData> GetSensorData(SensorBase::SensorType type)
    {
        ...
        case SensorBase::ST_Touch:
            return boost::shared_ptr<PySensorData>(new PyTouchSensorData(boost::static_pointer_cast<SensorBase::TouchGeomData>(_psensor->GetSensorGeometry()), boost::static_pointer_cast<SensorBase::TouchSensorData>(psensordata)));
    ...
    boost::shared_ptr<PySensorData> GetSensorGeometry(SensorBase::SensorType type)
    {
        ...
        case SensorBase::ST_Touch:
            return boost::shared_ptr<PySensorData>(new PyTouchSensorData(boost::static_pointer_cast<SensorBase::TouchGeomData>(_psensor->GetSensorGeometry())));
        case SensorBase::ST_Camera:
...
{
    {
        ...
        ;
         class_<PySensorBase::PyTouchSensorData, boost::shared_ptr<PySensorBase::PyTouchSensorData>, bases<PySensorBase::PySensorData> >("TouchSensorData", DOXY_CLASS(SensorBase::TouchSensorData),no_init)
        .def_readonly("positions",&PySensorBase::PyTouchSensorData::positions)
        .def_readonly("ranges",&PySensorBase::PyTouchSensorData::ranges)
        .def_readonly("intensity",&PySensorBase::PyTouchSensorData::intensity)
        ;
        ...
        .value("Touch",SensorBase::ST_Touch)
